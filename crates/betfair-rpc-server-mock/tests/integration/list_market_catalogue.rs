...
async fn list_market_catalogue() {
    ...
    let market_id = MarketId(CompactString::from("1.210878100".to_owned()));
    let result = client
        .send_request(
            list_market_catalogue::Parameters::builder()
                ...
                .filter(
                    MarketFilter::builder()
                        .event_type_ids(vec![EventTypeId(CompactString::from(
                            "31908334".to_owned(),
                        ))])
                        ...
                        .market_type_codes(vec![MarketType(CompactString::from(
                            "MATCH_ODDS".to_owned(),
                        ))])
    ...
    let market = MarketCatalogue {
        market_id: MarketId(CompactString::from("1.206502771".to_owned())),
        total_matched: num_ord!(0).into(),
        market_start_time: "2022-11-15T17:00:00.000Z".parse().ok(),
        event: Some(Event {
            country_code: Some(CountryCode(CompactString::from("GB".to_owned()))),
            id: Some(EventId(CompactString::from("31908334".to_owned()))),
            name: Some(CompactString::from("Atomeromu Szekszard Women v Olympiakos Piraeus BC ".to_owned())),
            open_date: Some("2022-11-15T17:00:00.000Z".parse().unwrap()),
            timezone: Some(CompactString::from("GMT".to_owned())),
            venue: None,
        }),
        event_type: Some(EventType {
            id: Some(EventTypeId(CompactString::from("1123123".to_owned()))),
            name: Some(CompactString::from("Basketball".to_owned())),
        }),
        ...
        description: Some(MarketDescription {
            ...
            settle_time: None,
            betting_type: CompactString::from("ODDS".to_owned()),
            turn_in_play_enabled: true,
            market_type: MarketType(CompactString::from("MATCH_ODDS".to_owned())),
            regulator: Some(CompactString::from("MALTA LOTTERIES AND GAMBLING AUTHORITY".to_owned())),
            market_base_rate: Some(num_ord!(2.0)),
            discount_allowed: Some(false),
            wallet: Some(CompactString::from("UK wallet".to_owned()),),
            rules: Some(
        ...
        market_name: CompactString::from("Moneyline".to_owned()),
    };
